...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr
{
	...
	if (self != nil)
	{
		...
		_containerView = [[UIView alloc] initWithFrame:[UIView fitCGRect:CGRectMake(0, 0, 320.0f, 480) isBackView:YES]];
		
		_transitionView = [[UIView alloc] initWithFrame:[UIView fitCGRect:CGRectMake(0, 0, 320.0f, 480 - kTabBarHeight)
                                                               isBackView:YES]];
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue] >= 7)
        {
            if (iPhone5)
            {
                ...
                _tabBar = [[LeveyTabBar alloc] initWithFrame:[UIView fitCGRect:CGRectMake(0, 480-kTabBarHeight-44, 320.0f, kTabBarHeight)
                                                                    isBackView:NO]
                                                buttonImages:arr];
            }
            else
        ...
        else
        {
            if (!iPhone5)
            {
                ...
                _tabBar = [[LeveyTabBar alloc] initWithFrame:[UIView fitCGRect:CGRectMake(0, 480-kTabBarHeight-54, 320.0f, kTabBarHeight)
                                                                    isBackView:NO]
                                                buttonImages:arr];
...
- (void)setTabBarTransparent:(BOOL)yesOrNo
{
	...
	{
		_transitionView.frame = [UIView fitCGRect:CGRectMake(0, 0, 320.0f, 480 - kTabBarHeight)
                                       isBackView:YES];
	}
}
...
- (void)hidesTabBar:(BOOL)yesOrNO animated:(BOOL)animated
{
	...
	if (animated == YES)
	{
		...
		if (yesOrNO == YES)
		{
			self.tabBar.frame = [UIView fitCGRect:CGRectMake(self.tabBar.frame.origin.x, self.tabBar.frame.origin.y + kTabBarHeight, self.tabBar.frame.size.width, self.tabBar.frame.size.height) isBackView:YES];
		}
		else 
		{
			self.tabBar.frame = [UIView fitCGRect:CGRectMake(self.tabBar.frame.origin.x, self.tabBar.frame.origin.y - kTabBarHeight, self.tabBar.frame.size.width, self.tabBar.frame.size.height)
                                       isBackView:YES];
		}
